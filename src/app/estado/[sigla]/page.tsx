...
// Define the list of states here (or import from a shared utility)
const usStates = [
...
// Function to generate static paths for each state
export async function generateStaticParams() {
  return usStates.map((state) => ({
    sigla: state.abbr.toLowerCase(), // Ensure lowercase as used in links
  }));
}

// Corrected typing for page props
// @ts-ignore - Suppressing persistent type error for deployment
const StatePage = async ({ params }: { params: { sigla: string } }) => {
  const { sigla } = params;
